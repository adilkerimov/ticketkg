...
class Specifics_model extends Crud
{
    
public $table = 'specifics'; //Имя таблицы	
public $idkey = 'id_specific'; //Имя ID
...
(
    ...
    (
      'field' => 'specific_name',
      'label' => 'Наименование набора характеристик',
      'rules' => 'required'
    ),
    array
    (
      'field' => 'id_specification0',
      'label' => 'По крайней мере одна спецификация должна быть добавлена',
      'rules' => 'required'
    ),
    array
    (
...
public function get_specific_groups()
{   
...
public function add_specific_binds()
{
    $data['specific_name'] = $this->input->post('specific_name');
    $data['date'] = $this->input->post('date');
    $data['status'] = 1;
    $this->db->insert('specifics',$data);
}

public function get_specific_binds()
{
    $this->db->order_by ('id_specific','desc');
    $this->db->limit (1);
    $query = $this->db->get('specifics');
    return $query->row_array();//Возвращаем массив с последними материалами
}

public function add_specific($binds)
{
    for ($i=0;$i<$this->input->post('counter');$i++)
    {
    $data['id_specific'] = $binds;
    $data['id_item'] = $this->input->post('id_specification'.$i);
    $data['date'] = $this->input->post('date');
    $this->db->insert('specific_binds',$data);
    }
}
