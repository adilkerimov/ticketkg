...
office@ticket.kg</div></div>
    ...
    <script type="text/javascript">
      ...
      $("#cont").keypress(function(e){
           ...
           var pid = $('#cont').val();
           var act = $('#id_item_act').val();
           $.post(link + "main/search_ticket", {id_item_pin: pid, id_item_act: act},
             function(data){
              if(data == 1){
              $('#screen').removeClass('btn-default').removeClass('btn-danger').addClass('btn-success').text('БИЛЕТ ОПЛАЧЕН');
              }
              else if(data == 2){
              $('#screen').removeClass('btn-default').removeClass('btn-success').addClass('btn-danger').text('БИЛЕТ ИСПОЛЬЗОВАН');
              }
              else if(data == 3){
              $('#screen').removeClass('btn-default').removeClass('btn-success').addClass('btn-warning').text('БИЛЕТ НА ДРУГОЕ МЕРОПРИЯТИЕ');
              }
              else if(data == 0){
              $('#screen').removeClass('btn-default').removeClass('btn-success').addClass('btn-danger').text('НЕИЗВЕСТНО');
              }
              else{
              $('#screen').removeClass('btn-default').removeClass('btn-success').addClass('btn-danger').text('Неизвестная ошибка!');
              alert(data);
              }
             });
           $(this).val('');
