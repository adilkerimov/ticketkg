...
class Pages extends Controller {
        ...
        public function userprizes()
	{
       ...
       {
       $page = 'userprizes';
       $data['main_info'] = $this->prizes_model->get_page($page);
       ...
       //$data['user_account'] = $this->prizes_model->user_account($just_get);
    ...
    public function withdrawal()
	{
       ...
       $user = $this->ion_auth->user()->row();
       $just_get = $user->id;
       $data['withdrawals'] = $this->prizes_model->withdrawal($just_get);      
       $data['user_account'] = $this->prizes_model->user_account($just_get);
       $data['first_name'] = $user->first_name;
       $data['last_name'] = $user->last_name;
       $data['phone'] = $user->phone;
       ...
       {
       $this->form_validation->set_rules($this->prizes_model->add_rules);
       if ($this->form_validation->run() == TRUE)
       {
       
       $money_win = $this->prizes_model->user_moneys($just_get);//призы
       $account_bal = $this->prizes_model->user_account($just_get);//баланс счета
       $money_doc = $this->prizes_model->user_docs($just_get);//кредитовые документы
       $money_with_doc = $this->prizes_model->user_with_docs($just_get);//дебетовые документы
       ...
       $user_debet = 5;
       $kt_acc = $this->prizes_model->get_account_id($user_debet);
       $doc['kredit_user'] = $user_debet;         
       $doc['kredit_bal'] = $kt_acc['balance'];
       $doc['debet_bal'] = $account_bal['balance'];
       
       $this->prizes_model->provodka_with($doc);    
       $this->prizes_model->kredit_with($doc);
       $this->prizes_model->debet_with($doc);
       $this->prizes_model->apply();
       $data['withdrawals'] = $this->prizes_model->withdrawal($just_get);
	...
	{  
       ...
       $limit = 50;
       $total = $this->prizes_model->count_by();;
       
       $settings = $this->pagination_lib->get_settings($total,$limit);
       $this->pagination->initialize($settings);
       $data['page_nav'] = $this->pagination->create_links();
       
       $data['all_user_prizes'] = $this->prizes_model->all_win_prizes($limit,$start_from);
